...
**★ 以下机制类关键词也触发本 skill**：**效应 / 霜渐 / 虚湮 / 光噪 / 聚爆 / 电磁 / 风蚀 / 偏谐 / 震谐 / 集谐 / 谐度破坏 / 偏移 / 干涉 / 响应 / 协奏能量 / 延奏技能 / 变奏技能 / 共鸣解放 / 共鸣能量 / 逆势回击 / 成功闪避 / 瘫痪 / 超频技 / 对策技 / 声骸技能 / 数据坞 / 共振度 / 孱弱 / 绝缘 / 强韧 / 击退 / 连击 / 精英怪 / 效应强化 / 异相残象 / 逆境深塔 / 疲劳值**（机制定义见 `references/mechanics.md`，全部为官方 WIKI 词条，含 entryId 可溯源）。
...
6. **延奏增益精确配对（通用 R3，配队核心机制链）**：配队的本质 = 主C 的输出类型精确命中队友的延奏增益。逐对核对「A 延奏 buff 类型 ∩ B 主C输出方式」（⚠️ 受第7条延奏接力机制约束：**接力型**的"命中"指轮换环中紧挨主C之前那位的延奏；**队伍/光环型**全队生效、不受切换限制）：
   - 主C 共鸣解放 → 配 鉴心（接力：共鸣解放+38%）、守岸人（队伍：全伤15%）、莫宁（队伍：全伤25%）
   - 主C 重击 → 配 莫特斐（接力：重击+38%）、尤诺（接力：重击+50%）
   - 主C 共鸣技能 → 配 桃祈（接力：共鸣技能+38%）、坎特蕾拉（接力：共鸣技能+25%）
   - 主C 湮灭 → 配 坎特蕾拉/弗洛洛（接力：湮灭+20%）、丹瑾（接力：湮灭+23%）
   - 主C 风蚀 → 配 夏空（队伍：风蚀伤害加深100%）
   - 主C 普攻 → 配 散华（接力：普攻+38%）
7. **延奏接力机制（通用 R9，配队底层机制；按作用范围分「接力型」/「队伍·光环型」）**：延奏技能（Outro）只在角色协奏能量满后**退场**时触发。增益分两类：**接力型**——只给下一个切换进场的角色，且该角色一切场增益立即消失；三人轮转成环，主C每次进场只吃得到环里排它前面那一位的接力延奏；**队伍/光环型**——全队生效，不绑定"下一个进场角色"，可与接力型并存叠加。推论：**接力型延奏拐一队最多一个**（两个接力型同队必浪费一个，还挤掉奶位），第二辅助位留给**队伍型增益**（守岸人/莫宁/夏空等全队生效、可与接力型叠加）+ 纯奶/机制拐/自带奶角色；唯一接力延奏拐选**覆盖面最大**的（如坎特蕾拉=共鸣技能+25%+湮灭+20%双命中弗洛洛 > 桃祈单命中）。**实例分类**：接力型=坎特蕾拉/桃祈/弗洛洛/鉴心/莫特斐/尤诺/散华/丹瑾；队伍/光环型=守岸人(全伤15%)/莫宁(全伤25%)/夏空(风蚀+100%)/卜灵。完整说明见 `references/endstate-matrix.md` R9。
8. **增益集中堆主C（通用 R4）**：一队结构 = 主C + 增益辅助（延奏命中主C）+ 奶。**辅助位优先选"能给主C增益"的角色，不塞第二个主C/副C补刀**——主C伤害最高，增益应全给它。⚠️ "集中"≠ 叠多个**接力型**延奏拐（受第7条约束接力型只挑一个），而是把唯一接力位选准 + **队伍型（守岸人/莫宁/夏空等）可随便叠** + 用奶位增伤/常驻机制补。
