...
用户提到以下任一关键词时激活：鸣潮 / 库街区 / 共鸣者 / 声骸 / 武器 / 武器投影 / 合鸣效果 / 全息战略 / 道具 / 材料 / 素材 / 角色攻略 / 攻略 / **终焉矩阵 / 矩阵配队 / 疲劳值 / 增幅回路 / 危机特勤**（终焉矩阵相关查询必读 `references/endstate-matrix.md`）。

**★ 以下机制类关键词也触发本 skill**：**效应 / 霜渐 / 虚湮 / 光噪 / 聚爆 / 电磁 / 风蚀 / 偏谐 / 震谐 / 集谐 / 谐度破坏 / 偏移 / 干涉 / 响应 / 协奏能量 / 延奏技能 / 变奏技能 / 共鸣解放 / 共鸣能量 / 逆势回击 / 成功闪避 / 瘫痪 / 超频技 / 对策技 / 声骸技能 / 数据坞 / 共振度**（机制定义见 `references/mechanics.md`，全部为官方 WIKI 词条，含 entryId 可溯源）。
...
**🚨 最高优先级原则（外部权威攻略 > 引擎增益标签）**：
1. **优先对齐外部权威攻略钦定的体系协同**：鸣潮配队的胜负手是**效应体系闭环**（如震谐、热熔、协同共鸣等），不是"增益标签数量"。若攻略明确钦定某体系队友组合（如爱弥斯的最佳震谐队友琳奈+莫宁），**该组合必须排在最前**——即使它的"增益主C清单"看起来没有双奶多。
2. **增益标签降级为辅助参考**：`buff_out/gen/off` 只是"关键词粗提取"（可能漏判/误判体系协同），**不能作为"哪队最强"的主依据**。它只用于判断"这队能不能用 / 值不值得补练"，不用于决定谁排第一。
3. **体系契合 > 通用增益**：通配安全牌（如守岸人"全伤害加深"）是"万金油可用"，但**配特定主C时不如体系内角色**。不要因为"通配增益多"就把双奶/通用辅排到体系队前面。
4. **必须核对用户实际拥有**：精排前先 `my roles` 确认角色池，攻略钦定队友若用户没有，用池内同体系角色替代并标注"人工替代"。

**🔄 通用配队规则（普通配队与终焉矩阵通用，矩阵规则见 references/endstate-matrix.md R1-R9；效应/偏谐/协奏等机制定义与官方出处见 references/mechanics.md）**：
5. **版本强度优先（通用 R1）**：主C优先选当前版本（3.x）或最近版本的强主C，1.0/2.0 老主C除非高命座（S2+）否则不做核心。判定依据 `wuthering.gg/tier-list` + 版本号，**probe 的「定位/伤害」标签不能替代版本强度**。老角色如椿（1.0）即使满练也不如 3.x 新主C。
6. **延奏增益精确配对（通用 R3，配队核心机制链）**：配队的本质 = 主C 的输出类型精确命中队友的延奏增益。逐对核对「A 延奏 buff 类型 ∩ B 主C输出方式」（⚠️ 受第7条延奏接力机制约束，"命中"指轮换环中紧挨主C之前那位的延奏）：
   ...
   - 主C 风蚀 → 配 夏空（风蚀伤害加深100%）
   - 主C 普攻 → 配 散华（普攻+38%）
7. **延奏接力机制（通用 R9，配队底层机制，违反即浪费辅助位）**：延奏技能（Outro）只在角色协奏能量满后**退场**时触发，增益**只给下一个切换进场的角色**，且该角色一切场增益立即消失；三人轮转成环，**主C每次进场只吃得到环里排它前面那一位的延奏**。推论：**一队只配一个延奏拐**（两个延奏拐同队必浪费一个，还挤掉奶位），第二辅助位留给纯奶/机制拐/自带奶角色；唯一延奏拐选**覆盖面最大**的（如坎特蕾拉=共鸣技能+25%+湮灭+20%双命中弗洛洛 > 桃祈单命中）。例外：增益来源是常驻机制（光环/减防/效应施加）而非延奏的，不受此限，可与延奏拐并存——配队前先确认增益**来源**是不是延奏技能。完整说明见 `references/endstate-matrix.md` R9。
8. **增益集中堆主C（通用 R4）**：一队结构 = 主C + 增益辅助（延奏命中主C）+ 奶。**辅助位优先选"能给主C增益"的角色，不塞第二个主C/副C补刀**——主C伤害最高，增益应全给它。⚠️ "集中"≠ 叠多个延奏拐（受第7条约束只挑一个），而是把唯一延奏位选准 + 用奶位增伤/常驻机制补。
9. **真实定位识别（通用 R5）**：wiki 标签 ≠ 实战定位。标「生存治疗」的可能是增伤拐/副C（如千咲=异常体系减防拐、莫宁=震谐奶、鉴心=共鸣解放+38%增伤），标「主力输出」的也可能增益型。配队前看 `伤害` 字段 + 主动效应 + 延奏原文。
10. **不因当前练度排除增益角色（通用 R7）**：4 星增益辅助/奶（鉴心/莫特斐/桃祈/白芷/散华/丹瑾等）即使当前等级低/没声骸，只要**满命（S6）+ 延奏增益命中主C**就值得上——武器声骸可跨角色换，配齐装备即可上场。等级/声骸是装备问题，不是机制问题。
...
- **纯奶位（维里奈/守岸人）是"保生存+通用增益"的安全牌，可作保底/过渡**，但**不应因为通配增益多就排到体系队前面**——除非目标主C确实缺少体系内队友，才考虑用奶位补位
...
- **现象**：`list <分类>` 直接打印到 stdout 时，含生僻字的角色名会显示为乱码（如"锁暝"的「暝」、"心"的角色名等），导致差集对比时把真实角色误判为"名单外"。
- **根因**：Windows console 默认 GBK 编码，wiki 返回的 UTF-8 罕用字无法映射；数据本身没问题，只是显示层丢失。
- **正确做法**：需要拿名单做**程序化对比**时，不要肉眼读 stdout——用 `--json` 输出重定向到文件后用 `json.load` 读取（UTF-8 原样无损），或写 `.py` 脚本在进程内调用 API 并全程 UTF-8 处理。
